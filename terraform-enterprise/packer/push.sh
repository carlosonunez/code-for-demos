#!/bin/bash
# ./push.sh <template> <vpc_id> <subnet_id> 
# Pushes this image using a Docker container (so that we don't have to
# install packer ourselves).

template="${1:?Please provide the Packer template to push from.}"
...
docker run --volume "$PWD:/packer" \
  ...
  --workdir "/packer" \
  "hashicorp/packer" push -var "aws_access_key=$AWS_ACCESS_KEY_ID" \
